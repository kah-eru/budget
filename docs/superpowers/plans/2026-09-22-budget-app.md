...
- Mobile performance targets: LCP <= 2.5 seconds, INP <= 200 ms, CLS <= 0.1; distinguish prelaunch lab measurements from field percentiles. Lab 2026-09-25 (Slow 4G, 4x CPU, local production build): LCP 0.9–2.1 s (noisy machine), CLS 0 on Overview/Timeline/Settings after the chart skeleton, initial JS 1.17 kB gzip; field data not collected. Initial app-delivered JavaScript budget remains 150 KiB compressed, including loaded React, Flowbite, Motion, Bklit, Kokonut, and their dependencies. Measure deferred bytes separately; compatibility and budget compliance are unverified.
