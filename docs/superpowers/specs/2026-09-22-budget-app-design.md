...
| Categorize by example (requested 2026-09-25) | Implemented 2026-09-25 | From a category, search transactions by keyword, tick the ones that belong and keep the keyword as a "name contains" rule; or, on one transaction, apply its category to other and future transactions with the same name. Ticked rows become manual choices; the rule makes future matches count toward that category's budget. |
