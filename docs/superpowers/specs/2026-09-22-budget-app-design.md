...
Manual transactions (integer cents, USD, expense/refund/income/transfer, pending) a this-month spending summary and per-workspace annotations (display name, classification override, note; no category yet) exist; imports, timeline and reports do not. Budgets, banks, jobs, AI, statements, deployment and live Manus SEO are not implemented. Row-locking checks pass on hosted PostgreSQL (Neon, synthetic data); load and multi-instance deployment remain unverified. Current evidence and known issues are in [development setup and verification](../../development.md).
